...
class UserMainMapScreenViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private func startOnlineBusTimer() {
        ...
        onlineBusTimer = Timer.scheduledTimer(withTimeInterval: 10, repeats: true) { _ in
            self.observeOnlineBusOperators(selectedRoute: BusRoute.defaultRoute)
        }
    }
            ...
            DispatchQueue.global(qos: .background).async {
                do {
                    if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                        ...
                        // Filter operators based on route and reverse logic
                        for (busID, details) in json {
                            if let busData = details as? [String: Any],
                               ...
                               let routeInterchange = busData["routeInterchange"] as? Bool {

                                // Get the route matching the busRoute
                                                         if let route = BusRoute.allRoutes.first(where: { $0.routeNumber == busRoute }) {

                                                             // Filter based on selected route and reverse logic
                                                             if (routeInterchange && route.isReverse) || (!routeInterchange && !route.isReverse) {
